    ...
    internal sealed class EpicSocketManager : IDisposable
    {
        ...
        async void ProcessIncomingMessages()
        {
            try
            {
                while (true)
                {
                    for (int chNum = 0; chNum <= this._epicOptions.Channels.Length; chNum++)
                    {
                        while (this.DataAvailable(out ProductUserId clientUserID, out byte[] receiveBuffer, (byte)chNum,
                                   out SocketId socket))
                        {

                            switch (chNum == this._epicOptions.Channels.Length)
                            {
                                case true:
                                    this.OnReceiveInternalData((InternalMessage)receiveBuffer[0], clientUserID, socket);
                                    break;
                                case false:
                                    this.OnReceiveData(receiveBuffer, clientUserID, chNum);
                                    break;
                            }
                        }
                    }
